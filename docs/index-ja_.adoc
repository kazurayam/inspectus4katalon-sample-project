...
link:https//qiita.com/[Qiita]の読者ならこのGitHubプロジェクトを `git clone` してWindows PC上のKatalon Studioで動かすことができるでしょう。その方法だと応用が効きません。あなたがKatalonプロジェクトを作り、自分のwebサイトを画面確認する作業を自動化するのに役立たない。ここでは `git clone` するのではなく別の方法を紹介します。ビルドツール link:https://gradle.org/[Gradle] を使います。
