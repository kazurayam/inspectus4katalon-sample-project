...
Katalon Studioは link:https://katalon.com/resources-center/blog/handle-selenium-wait[Smart Wait] という小技を備えています。Smart Waitは場合によって悪さをします。本当なら３０秒で終わるはずのテストがSmart Waitのせいで２０分経過してまだ終わらないなんてことが起きる。ところがデフォルトの設定がSmart WaitをONにしています。OFFにしましょう。
...
これを実行するにはウインドウの上部に配置された緑色や矢印のボタン image:https://kazurayam.github.io/inspectus4katalon-sample-project/images/run_katalon_test.png[run button]を押します。
